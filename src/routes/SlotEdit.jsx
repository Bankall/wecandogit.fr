...
export default function SlotEdit() {
	...
	const [slotLabel, setSlotLabel] = useState(false);
	const [formData, setFormData] = useState(false);

	useEffect(() => {
		const fetch = async () => {
			try {
				const response = await axios.get(`/slot/${params.id}`, { withCredentials: true });
					...
					setFormData(
						Slot.map(row => {
							...
							if (typeof response.data[item.name] !== "undefined") {
								...
								if (item.name === "label") {
									setSlotLabel(response.data[item.name]);
								}
							}
						...
						<div className='box big-box'>
							<FormikWrapper
								...
								onSubmit={async ({ values, setCustomIsSubmitting, setSubmitionError, setSubmitionFeedback }) => {
									...
									try {
										const response = await axios.put(`/slot/${params.id}`, values, { withCredentials: true });
